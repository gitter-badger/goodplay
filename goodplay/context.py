...
class GoodplayContext(object):
    ...
    def use_local_roles(self):
        use_local_roles = self.pytestconfig.getoption('use_local_roles')
        if not use_local_roles:
            use_local_roles = self.pytestconfig.getini('use_local_roles')
        return use_local_roles
