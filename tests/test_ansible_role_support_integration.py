...
@pytest.mark.parametrize('config_with', ['cmdline_option', 'ini_file'])
def test_when_use_local_roles_is_enabled_ansible_path_is_considered(
        testdir, monkeypatch, config_with):
    monkeypatch.setattr(
        'ansible.constants.DEFAULT_ROLES_PATH',
    ...
    if config_with == 'cmdline_option':
        result = testdir.inline_run('-s', '--use-local-roles')
    elif config_with == 'ini_file':
        smart_create(testdir.tmpdir, '''
        ## pytest.ini
        [pytest]
